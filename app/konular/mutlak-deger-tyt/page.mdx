import { topicMetadata } from "@/lib/seo";

export const metadata = topicMetadata("mutlak-deger-tyt");

<Konu slug="mutlak-deger-tyt" />

<p className="lead">Mutlak değer, bir sayının **sayı doğrusunda sıfıra olan uzaklığıdır**; bu yüzden işareti ne olursa olsun sonuç **daima sıfır ya da pozitiftir**. Bu konu, mutlak değerin tanımından temel özelliklerine, mutlak değerli denklem ve eşitsizliklerin çözümüne kadar TYT'de doğrudan karşına çıkan her şeyi kapsar.</p>

## 1. Tanım

Bir $x$ gerçek sayısının mutlak değeri parçalı (cebirsel) olarak şöyle tanımlanır:

$$\lvert x\rvert=\begin{cases}x,& x\ge 0\\ -x,& x<0\end{cases}$$

Sözel olarak: $\lvert x\rvert$, $x$ sayısının sayı doğrusunda **sıfıra uzaklığıdır**. Uzaklık negatif olamayacağından her zaman $\lvert x\rvert\ge 0$ olur. Örneğin $\lvert 5\rvert=5$ ve $\lvert -5\rvert=-(-5)=5$.

> **Dikkat:** $x<0$ olduğunda $\lvert x\rvert=-x$ ifadesi **pozitif** bir sayıdır; çünkü negatif bir sayının işaretini değiştirmek onu pozitif yapar. "$-x$" görüp "negatif" sanmak en sık yapılan yanılgıdır.

## 2. Temel Özellikler

| Özellik | Açıklama |
| --- | --- |
| $\lvert x\rvert\ge 0$ | Mutlak değer asla negatif olamaz |
| $\lvert -x\rvert=\lvert x\rvert$ | Zıt işaretli sayıların mutlak değeri eşittir |
| $\lvert a\cdot b\rvert=\lvert a\rvert\cdot\lvert b\rvert$ | Çarpımın mutlak değeri = mutlak değerlerin çarpımı |
| $\lvert a-b\rvert=\lvert b-a\rvert$ | İki sayı arasındaki uzaklık simetriktir |

Ayrıca $\lvert x\rvert=0$ olması için tek koşul $x=0$ olmasıdır.

## 3. Mutlak Değerli Denklem

Temel kural: $a\ge 0$ olmak üzere

$$\lvert x\rvert=a \;\Rightarrow\; x=a \ \text{veya}\ x=-a$$

Yani mutlak değeri $a$ olan **iki** sayı vardır: $+a$ ve $-a$. Genel olarak içerideki ifade $f(x)$ ise:

$$\lvert f(x)\rvert=a \;\Rightarrow\; f(x)=a \ \text{veya}\ f(x)=-a$$

Kısaca $f(x)=\pm a$ yazılır ve her iki denklem ayrı ayrı çözülür.

**Önemli:** Sağ taraf **negatif** ise ($a<0$) denklemin **çözümü yoktur**; çünkü mutlak değer negatif bir sonuca eşit olamaz.

## 4. Mutlak Değerli Eşitsizlik

İki temel yapı vardır ve birbirine karıştırılmamalıdır:

$$\lvert x\rvert < a \;\Rightarrow\; -a < x < a \quad(\text{tek aralık, "arada"})$$

$$\lvert x\rvert > a \;\Rightarrow\; x < -a \ \text{veya}\ x > a \quad(\text{iki ayrı aralık, "dışında"})$$

Buradaki $a$ pozitif kabul edilir. $\le$ ve $\ge$ durumlarında uç noktalar çözüme dâhil edilir (eşitlik korunur).

<Ornek>

<Soru>

$\lvert -5\rvert$ ve $\lvert 3-7\rvert$ değerlerini hesaplayınız.

</Soru>

<Cozum>

1. $\lvert -5\rvert$: $-5<0$ olduğundan $\lvert -5\rvert=-(-5)=5$.

2. $\lvert 3-7\rvert=\lvert -4\rvert$: $-4<0$ olduğundan $\lvert -4\rvert=4$.

<Cevap>$\lvert -5\rvert=5$ ve $\lvert 3-7\rvert=4$.</Cevap>

</Cozum>

</Ornek>

<Ornek>

<Soru>

$\lvert 2x-4\rvert=6$ denkleminin çözüm kümesini bulunuz.

</Soru>

<Ipucu>

İçerideki ifadeyi $\pm 6$'ya eşitle: $2x-4=6$ veya $2x-4=-6$. İki denklemi ayrı çöz.

</Ipucu>

<Cozum>

1. Kuralı uygula: $2x-4=6$ veya $2x-4=-6$.

2. Birinci denklem: $2x=10 \Rightarrow x=5$.

3. İkinci denklem: $2x=-2 \Rightarrow x=-1$.

<Cevap>$x=5$ veya $x=-1$; yani çözüm kümesi $\{-1,\,5\}$.</Cevap>

</Cozum>

</Ornek>

<Ornek>

<Soru>

$\lvert x-1\rvert < 3$ eşitsizliğini sağlayan $x$ değerlerini bulunuz.

</Soru>

<Cozum>

1. "Arada" kuralını uygula: $-3 < x-1 < 3$.

2. Her tarafa $1$ ekle: $-3+1 < x < 3+1$.

3. Sonuç: $-2 < x < 4$.

<Cevap>$-2 < x < 4$, yani $x\in(-2,\,4)$.</Cevap>

</Cozum>

</Ornek>

<Ornek>

<Soru>

$\lvert x+2\rvert \ge 5$ eşitsizliğinin çözüm kümesini bulunuz.

</Soru>

<Ipucu>

"Dışında" yapısı: $\lvert f(x)\rvert\ge a$ ise $f(x)\ge a$ veya $f(x)\le -a$. İki aralığı ayrı yaz.

</Ipucu>

<Cozum>

1. "Dışında" kuralını uygula: $x+2\ge 5$ veya $x+2\le -5$.

2. Birinci durum: $x\ge 3$.

3. İkinci durum: $x\le -7$.

<Cevap>$x\ge 3$ veya $x\le -7$; yani $x\in(-\infty,\,-7]\cup[3,\,\infty)$.</Cevap>

</Cozum>

</Ornek>

<Ornek>

<Soru>

$\lvert 3x\rvert=12$ denklemini çözünüz.

</Soru>

<Cozum>

1. $\lvert 3x\rvert=12 \Rightarrow 3x=12$ veya $3x=-12$.

2. Birinci denklem: $x=4$.

3. İkinci denklem: $x=-4$.

<Cevap>$x=\pm 4$, yani çözüm kümesi $\{-4,\,4\}$.</Cevap>

</Cozum>

</Ornek>

<Ornek>

<Soru>

$\lvert x\rvert=-3$ denkleminin çözümü var mıdır?

</Soru>

<Cozum>

1. Mutlak değer her zaman $\lvert x\rvert\ge 0$'dır; yani sonucu asla negatif olamaz.

2. Sağ taraf $-3<0$ olduğundan eşitlik hiçbir $x$ için sağlanamaz.

<Cevap>Çözüm yoktur; çözüm kümesi boş kümedir ($\varnothing$).</Cevap>

</Cozum>

</Ornek>

## Çözümlü Sorular

<Ornek>

<Soru>

$\lvert -3\rvert + \lvert 2-9\rvert - \lvert -4\rvert$ işleminin sonucu kaçtır?

</Soru>

<Cozum>

1. Her terimi ayrı ayrı hesapla: $\lvert -3\rvert=3$.

2. $\lvert 2-9\rvert=\lvert -7\rvert=7$.

3. $\lvert -4\rvert=4$.

4. Yerine yaz: $3+7-4=6$.

<Cevap>$6$</Cevap>

</Cozum>

</Ornek>

<Ornek>

<Soru>

$\lvert x-3\rvert=2x$ denkleminin çözüm kümesini bulunuz.

</Soru>

<Cozum>

1. Sağ taraf bir mutlak değere eşit olduğundan $2x\ge 0$, yani $x\ge 0$ olmalıdır.

2. $x-3=2x$ veya $x-3=-2x$ yaz.

3. Birinci denklem: $x-3=2x \Rightarrow -3=x$. Bu kök $x\ge 0$ koşulunu sağlamaz, elenir.

4. İkinci denklem: $x-3=-2x \Rightarrow 3x=3 \Rightarrow x=1$. Bu kök koşulu sağlar.

5. Kontrol: $\lvert 1-3\rvert=2$ ve $2x=2$; eşitlik sağlanır.

<Cevap>$x=1$, yani çözüm kümesi $\{1\}$.</Cevap>

</Cozum>

</Ornek>

<Ornek>

<Soru>

$\lvert 2x-1\rvert \le 5$ eşitsizliğini sağlayan tam sayı $x$ değerlerinin toplamı kaçtır?

</Soru>

<Cozum>

1. "Arada" kuralını uygula: $-5 \le 2x-1 \le 5$.

2. Her tarafa $1$ ekle: $-4 \le 2x \le 6$.

3. Her tarafı $2$'ye böl: $-2 \le x \le 3$.

4. Tam sayılar: $-2,-1,0,1,2,3$. Toplam: $-2-1+0+1+2+3=3$.

<Cevap>$3$</Cevap>

</Cozum>

</Ornek>

<Ornek>

<Soru>

$2 < x < 5$ olmak üzere $\lvert x-2\rvert + \lvert x-5\rvert$ ifadesinin değeri nedir?

</Soru>

<Ipucu>

Verilen aralıkta $x-2$ ile $x-5$ ifadelerinin işaretlerini ayrı ayrı belirle.

</Ipucu>

<Cozum>

1. $2 < x$ olduğundan $x-2>0$, dolayısıyla $\lvert x-2\rvert=x-2$.

2. $x < 5$ olduğundan $x-5<0$, dolayısıyla $\lvert x-5\rvert=-(x-5)=5-x$.

3. Topla: $(x-2)+(5-x)=3$.

<Cevap>$3$</Cevap>

</Cozum>

</Ornek>

<Ornek>

<Soru>

$\lvert x-4\rvert = x-4$ eşitliğini sağlayan $x$ değerlerinin çözüm kümesini bulunuz.

</Soru>

<Cozum>

1. $\lvert a\rvert=a$ eşitliği yalnızca $a\ge 0$ olduğunda sağlanır.

2. Burada $a=x-4$ olduğundan $x-4\ge 0$ olmalıdır.

3. Buradan $x\ge 4$ bulunur.

<Cevap>$x\ge 4$, yani $x\in[4,\,\infty)$.</Cevap>

</Cozum>

</Ornek>

<Ornek>

<Soru>

$\lvert 3x+1\rvert = \lvert x-5\rvert$ denkleminin köklerinin toplamı kaçtır?

</Soru>

<Ipucu>

İki mutlak değer eşitse, içler ya eşittir ya da zıt işaretlidir: $3x+1=x-5$ veya $3x+1=-(x-5)$.

</Ipucu>

<Cozum>

1. Birinci durum: $3x+1=x-5 \Rightarrow 2x=-6 \Rightarrow x=-3$.

2. İkinci durum: $3x+1=-(x-5) \Rightarrow 3x+1=-x+5 \Rightarrow 4x=4 \Rightarrow x=1$.

3. Kontrol $x=-3$: $\lvert -8\rvert=8$ ve $\lvert -8\rvert=8$; sağlanır.

4. Kontrol $x=1$: $\lvert 4\rvert=4$ ve $\lvert -4\rvert=4$; sağlanır.

5. Köklerin toplamı: $-3+1=-2$.

<Cevap>Kökler $-3$ ve $1$; toplamları $-2$.</Cevap>

</Cozum>

</Ornek>

<Ornek>

<Soru>

$2 < \lvert x-1\rvert < 5$ eşitsizliğinin çözüm kümesini bulunuz.

</Soru>

<Cozum>

1. Eşitsizliği iki parçaya ayır: $\lvert x-1\rvert < 5$ ve $\lvert x-1\rvert > 2$.

2. $\lvert x-1\rvert < 5$ ("arada"): $-5 < x-1 < 5 \Rightarrow -4 < x < 6$.

3. $\lvert x-1\rvert > 2$ ("dışında"): $x-1 > 2$ veya $x-1 < -2$, yani $x>3$ veya $x<-1$.

4. İki sonucun kesişimini al: $-4 < x < -1$ veya $3 < x < 6$.

<Cevap>$x\in(-4,\,-1)\cup(3,\,6)$.</Cevap>

</Cozum>

</Ornek>

## Sınav Tarzı Sorular

> Aşağıdaki sorular, ÖSYM'nin TYT Temel Matematik'te sorduğu **çok kavramlı, dolaylı (5 şıklı)** soru tarzına örnek olarak **özgün biçimde** hazırlanmıştır.

<Ornek>

<Soru>

$\lvert x-3\rvert=7$ denkleminin köklerinin toplamı kaçtır?

A) $2$ · B) $4$ · C) $6$ · D) $8$ · E) $10$

</Soru>

<Cozum>

1. $x-3=7\Rightarrow x=10$ veya $x-3=-7\Rightarrow x=-4$.

2. Toplam: $10+(-4)=6$.

<Cevap>C) $6$</Cevap>

</Cozum>

</Ornek>

<Ornek>

<Soru>

$\lvert 2x+1\rvert=9$ denkleminin köklerinin toplamı kaçtır?

A) $-5$ · B) $-3$ · C) $-1$ · D) $1$ · E) $3$

</Soru>

<Cozum>

1. $2x+1=9\Rightarrow x=4$ veya $2x+1=-9\Rightarrow x=-5$.

2. Toplam: $4+(-5)=-1$.

<Cevap>C) $-1$</Cevap>

</Cozum>

</Ornek>

<Ornek>

<Soru>

$\lvert x-2\rvert < 5$ eşitsizliğini sağlayan tam sayıların sayısı kaçtır?

A) $7$ · B) $8$ · C) $9$ · D) $10$ · E) $11$

</Soru>

<Ipucu>

“Arada” yapısı: $-5 < x-2 < 5$; tam sayıları sayarken uçları dahil etme.

</Ipucu>

<Cozum>

1. $-5 < x-2 < 5\Rightarrow -3 < x < 7$.

2. Tam sayılar: $-2,-1,0,1,2,3,4,5,6$ → $9$ adet.

<Cevap>C) $9$</Cevap>

</Cozum>

</Ornek>

## Sık Yapılan Hatalar

- **Tek kök almak:** $\lvert x\rvert=a$ denkleminde yalnızca $x=a$ yazıp $x=-a$ kökünü unutmak. Mutlak değerli denklemde (sağ taraf pozitifse) **iki kök** vardır: $\pm a$.
- **Eşitsizlik yapılarını karıştırmak:** $\lvert x\rvert < a$ ("arada", tek aralık $-a<x<a$) ile $\lvert x\rvert > a$ ("dışında", iki ayrı aralık) çözümlerini birbirinin yerine kullanmak.
- **Çözümsüzlüğü görmemek:** $\lvert x\rvert=$ (negatif sayı) biçimindeki denklemin çözümsüz olduğunu fark etmeyip cebirsel işlem yapmaya çalışmak.

## Sınav İpucu

İki yapıyı tek cümleyle hatırla: $\lvert f(x)\rvert < a$ demek $f(x)$ değerinin $-a$ ile $a$ **arasında** kalması ($-a<f(x)<a$); $\lvert f(x)\rvert > a$ demek ise $f(x)$ değerinin bu aralığın **dışında** olması (iki ayrı aralık) demektir. Eşitsizliğe çözmeden önce "arada mı, dışında mı?" diye sor; yön doğru seçilince işlemler kendiliğinden gelir.

<KonuNav slug="mutlak-deger-tyt" />
